...
class ClientModel(Model):
    ...
    def create_model(self):
        ...
        def residual_block(inputs, output_channel, block_stride=1, name=''):
            if block_stride == 2:
                shortcut_conv = conv2d(inputs, output_channel, kernel_size=1, stride=block_stride, activation=False, name=name+'shortcut')
            else:
                shortcut_conv = inputs
            net = conv2d(inputs, output_channel, kernel_size=3, stride=block_stride, name=name+'_1')
            net = conv2d(net, output_channel, kernel_size=3, stride=1, activation=False, name=name+'_2')
        ...
        net = residual_block(net, 128, block_stride=2, name='conv3_1')
        net = residual_block(net, 128, block_stride=1, name='conv3_2')

        net = residual_block(net, 256, block_stride=2, name='conv4_1')
        net = residual_block(net, 256, block_stride=1, name='conv4_2')

        net = residual_block(net, 512, block_stride=2, name='conv5_1')
