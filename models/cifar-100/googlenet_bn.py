...
class ClientModel(Model):
    ...
    def create_model(self):
        ...
        def conv2d(inputs, output_channels, kernel_size, stride, name='', bn=False):
        ...
        def inception_module(inputs, ch1, ch3_red, ch3, ch5_red, ch5, pool_proj, name='', bn=False):
            branch1 = conv2d(inputs, ch1, 1, 1, name=name + '_branch1', bn=bn)

            branch2 = conv2d(inputs, ch3_red, 1, 1, name=name + '_branch2_0', bn=bn)
            branch2 = conv2d(branch2, ch3, 3, 1, name=name + '_branch2_1', bn=bn)

            branch3 = conv2d(inputs, ch5_red, 1, 1, name=name + '_branch3_0', bn=bn)
            branch3 = conv2d(branch3, ch5, 5, 1, name=name + '_branch3_1', bn=bn)

            branch4 = maxpooling2d(inputs, 3, stride=1, name=name + '_branch4_0')
            branch4 = conv2d(branch4, pool_proj, 1, 1, name=name + '_branch4_1', bn=bn)
        ...
        net = inception_module(net, 128, 128, 256, 24, 64, 64, 'inception_4c', True)
        ...
        net = inception_module(net, 384, 192, 384, 48, 128, 128, 'inception_5b', True)
